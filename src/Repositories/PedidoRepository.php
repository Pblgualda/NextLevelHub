...
class PedidoRepository{
    ...
    {
        try{
            $sql = "Select * from pedidos WHERE usuario_id = :usuario_id ORDER BY fecha_pedido DESC, id DESC";
            ...
            throw new RuntimeException(
                "Error al obtener los pedidos del usuario: {$e->getMessage()}",
                previous: $e
            );
        }
    }

    public function findByIdAndUsuarioId(int $id, int $usuarioId): ?Pedido
    {
        try{
            $sql = "Select * from pedidos WHERE id = :id AND usuario_id = :usuario_id LIMIT 1";
            $this->conexion->ejecutar($sql, [
                ':id' => ['valor' => $id],
                ':usuario_id' => ['valor' => $usuarioId],
            ]);
